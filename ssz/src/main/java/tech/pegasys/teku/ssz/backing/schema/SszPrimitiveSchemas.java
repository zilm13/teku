...
import tech.pegasys.teku.ssz.backing.view.SszPrimitives.SszUInt256;
...
public interface SszPrimitiveSchemas {
  ...
  AbstractSszPrimitiveSchema<SszUInt256> UINT256_SCHEMA =
      new AbstractSszPrimitiveSchema<>(256) {
        @Override
        public SszUInt256 createFromLeafBackingNode(LeafDataNode node, int internalIndex) {
          Bytes leafNodeBytes = node.getData();
          Bytes elementBytes = leafNodeBytes.slice(internalIndex * 32, 32);
          return SszUInt256.fromBI(toBigIntLE(elementBytes.toArray()));
        }
  ...
  private static BigInteger toBigIntLE(byte[] arr) {
    byte[] rev = new byte[arr.length + 1];
    for (int i = 0, j = arr.length; j > 0; i++, j--) rev[j] = arr[i];
