...
class Phase1Spec(val DATA: DataObjectFactory, val SSZ: SSZObjectFactory) {
  ...
  /*
    ...
    */
  fun is_valid_indexed_attestation(
    state: BeaconState,
    ...
    if (len(attestation.custody_bits_blocks) == 0uL) {
      ...
      for ((i, custody_bits) in enumerate(attestation.custody_bits_blocks)) {
        ...
        for ((participant, abit, cbit) in zip(
          ...
          if (abit) {
            all_pubkeys.append(state.validators[participant].pubkey)
            var attestation_wrapper = DATA.AttestationCustodyBitWrapper(
              attestation_data_root = hash_tree_root(attestation.data),
              block_index = i.toULong(),
              bit = cbit
            )
      ...
      return bls.AggregateVerify(
        zip(all_pubkeys, all_signing_roots),
  ...
  fun compute_shuffled_index(
    ...
    for (current_round in range(SHUFFLE_ROUND_COUNT)) {
      val pivot = (bytes_to_int(
        hash((seed + int_to_bytes(current_round, length = 1uL))).slice(
          0uL,
          8uL
        )
      ) % index_count)
      ...
      val source = hash(
        ((seed + int_to_bytes(current_round, length = 1uL)) + int_to_bytes(
          (position / 256uL),
          length = 4uL
        ))
      )
  ...
  fun compute_proposer_index(
    ...
    while (true) {
      var candidate_index =
        indices[compute_shuffled_index(ValidatorIndex((i % len(indices))), len(indices), seed)]
  ...
  /*
    Return the committee corresponding to ``indices``, ``seed``, ``index``, and committee ``tech.pegasys.teku.phase1.onotole.pylib.count``.
    */
  fun compute_committee(
    indices: Sequence<ValidatorIndex>,
    seed: Bytes32,
    index: uint64,
    count: uint64
  ...
  fun compute_fork_digest(current_version: Version, genesis_validators_root: Root): ForkDigest {
    return ForkDigest(
      compute_fork_data_root(current_version, genesis_validators_root).slice(
        0uL,
        4uL
      )
    )
  ...
  fun compute_domain(
    ...
    fork_version: Version? = null,
    genesis_validators_root: Root? = null
  ): Domain {
    var fork_data_root = compute_fork_data_root(
      fork_version
  ...
  fun compute_signing_root(ssz_object: SSZObject, domain: Domain): Root {
    var domain_wrapped_object =
      DATA.SigningRoot(object_root = hash_tree_root(ssz_object), domain = domain)
    return hash_tree_root(domain_wrapped_object)
  }

  /*
    Return the current epoch.o
    */
  ...
  fun get_active_validator_indices(state: BeaconState, epoch: Epoch): Sequence<ValidatorIndex> {
    return enumerate(state.validators).filter { (_, v) -> is_active_validator(v, epoch) }
      .map { (i, _) -> ValidatorIndex(i) }.toMutableList()
  }
  ...
  fun get_seed(state: BeaconState, epoch: Epoch, domain_type: DomainType): Bytes32 {
    var mix = get_randao_mix(
      state,
      Epoch((((epoch + EPOCHS_PER_HISTORICAL_VECTOR) - MIN_SEED_LOOKAHEAD) - 1uL))
    )
  ...
  fun get_beacon_proposer_index(state: BeaconState): ValidatorIndex {
    ...
    var seed = hash(
      (get_seed(state, epoch, DOMAIN_BEACON_PROPOSER) + int_to_bytes(
        state.slot,
        length = 8uL
      ))
    )
  ...
  fun get_total_balance(state: BeaconState, indices: Set<ValidatorIndex>): Gwei {
    return Gwei(
      max(
        ...
        sum(indices.map { index -> state.validators[index].effective_balance }.toMutableList())
      )
    )
  }
  ...
  fun get_total_active_balance(state: BeaconState): Gwei {
    return get_total_balance(
      state,
      set(get_active_validator_indices(state, get_current_epoch(state)))
    )
  }
  ...
  fun get_domain(state: BeaconState, domain_type: DomainType, epoch: Epoch? = null): Domain {
    ...
    var fork_version =
      if (epoch_ < state.fork.epoch) state.fork.previous_version else state.fork.current_version
    return compute_domain(domain_type, fork_version, state.genesis_validators_root)
  }

  fun get_indexed_attestation(
    beacon_state: BeaconState,
    attestation: Attestation
  ): IndexedAttestation {
    var committee = get_beacon_committee(
      beacon_state,
      attestation.data.slot,
      attestation.data.index
    ).toMutableList()
    return DATA.IndexedAttestation(
      committee = SSZ.SSZList(
  ...
  /*
    Return the tech.pegasys.teku.phase1.onotole.pylib.set of attesting indices corresponding to ``data`` and ``bits``.
    */
  fun get_attesting_indices(
    state: BeaconState,
  ...
  fun decrease_balance(state: BeaconState, index: ValidatorIndex, delta: Gwei): Unit {
    state.balances[index] =
      if ((delta > state.balances[index])) 0uL else (state.balances[index] - delta)
  }
  ...
  fun initiate_validator_exit(state: BeaconState, index: ValidatorIndex): Unit {
    ...
    var exit_epochs =
      state.validators.filter { v -> (v.exit_epoch != FAR_FUTURE_EPOCH) }.map { v -> v.exit_epoch }
        .toMutableList()
    var exit_queue_epoch =
      max((exit_epochs + mutableListOf(compute_activation_exit_epoch(get_current_epoch(state)))))
    var exit_queue_churn =
      len(state.validators.filter { v -> (v.exit_epoch == exit_queue_epoch) }.map { v -> v }
        .toMutableList())
    ...
    validator.withdrawable_epoch =
      Epoch((validator.exit_epoch + MIN_VALIDATOR_WITHDRAWABILITY_DELAY))
  }
  ...
  fun slash_validator(
    ...
    whistleblower_index: ValidatorIndex? = null
  ): Unit {
    var epoch = get_current_epoch(state)
    ...
    validator.withdrawable_epoch =
      max(validator.withdrawable_epoch, Epoch((epoch + EPOCHS_PER_SLASHINGS_VECTOR)))
  ...
  fun initialize_beacon_state_from_eth1(
    ...
    var state = DATA.BeaconState(
      genesis_time = ((eth1_timestamp - (eth1_timestamp % MIN_GENESIS_DELAY)) + (2uL * MIN_GENESIS_DELAY)),
      fork = fork,
      eth1_data = DATA.Eth1Data(
        block_hash = eth1_block_hash,
        deposit_count = len(deposits)
      ),
      latest_block_header = DATA.BeaconBlockHeader(
        body_root = hash_tree_root(DATA.BeaconBlockBody())
      ),
      randao_mixes = SSZ.SSZVector(
        Bytes32::class,
        MutableList(EPOCHS_PER_HISTORICAL_VECTOR.toInt()) { eth1_block_hash })
    )
    var leaves = list(map({ deposit -> deposit.data }, deposits))
    for ((index, deposit) in enumerate(deposits)) {
      var deposit_data_list = SSZ.SSZList(
        DepositData::class, 2uL.pow(DEPOSIT_CONTRACT_TREE_DEPTH),
        leaves.slice(0uL, (index.toULong() + 1uL)).toMutableList()
      )
      val eth1_data = state.eth1_data
      eth1_data.deposit_root = hash_tree_root(deposit_data_list)
    ...
    for ((index, validator) in enumerate(state.validators)) {
      ...
      validator.effective_balance =
        min((balance - (balance % EFFECTIVE_BALANCE_INCREMENT)), MAX_EFFECTIVE_BALANCE)
  ...
  fun verify_block_signature(state: BeaconState, signed_block: SignedBeaconBlock): pybool {
    ...
    var signing_root =
      compute_signing_root(signed_block.message, get_domain(state, DOMAIN_BEACON_PROPOSER))
  ...
  fun process_slot(state: BeaconState): Unit {
    ...
    if ((state.latest_block_header.state_root == Bytes32())) {
      val latest_block_header = state.latest_block_header
      latest_block_header.state_root = previous_state_root
      state.latest_block_header = latest_block_header
    }
  ...
  fun get_matching_source_attestations(
    state: BeaconState,
    epoch: Epoch
  ...
  fun get_matching_target_attestations(
    state: BeaconState,
    epoch: Epoch
  ): Sequence<PendingAttestation> {
    return get_matching_source_attestations(
      state,
      epoch
    ).filter { a -> (a.data.target.root == get_block_root(state, epoch)) }.map { a -> a }
      .toMutableList()
  }

  fun get_matching_head_attestations(
    state: BeaconState,
    epoch: Epoch
  ): Sequence<PendingAttestation> {
    return get_matching_target_attestations(
      state,
      epoch
    ).filter { a -> (a.data.beacon_block_root == get_block_root_at_slot(state, a.data.slot)) }
      .map { a -> a }.toMutableList()
  }
  ...
  /*
    Return the combined effective balance of the tech.pegasys.teku.phase1.onotole.pylib.set of unslashed validators participating in ``attestations``.
    Note: ``get_total_balance`` returns ``EFFECTIVE_BALANCE_INCREMENT`` Gwei minimum to avoid divisions by zero.
    */
  ...
  fun process_justification_and_finalization(state: BeaconState): Unit {
    ...
    if (((get_attesting_balance(
      ...
      ) * 3uL) >= (get_total_active_balance(state) * 2uL))
    ) {
      state.current_justified_checkpoint = DATA.Checkpoint(
    ...
    if (((get_attesting_balance(
      ...
      state.current_justified_checkpoint =
        DATA.Checkpoint(epoch = current_epoch, root = get_block_root(state, current_epoch))
    ...
    if (all(
      ...
      ) && ((old_previous_justified_checkpoint.epoch + 3uL) == current_epoch)
    ) {
    ...
    if (all(
      ...
      ) && ((old_previous_justified_checkpoint.epoch + 2uL) == current_epoch)
    ) {
      state.finalized_checkpoint = old_previous_justified_checkpoint
    }
    if (all(
      ...
      ) && ((old_current_justified_checkpoint.epoch + 2uL) == current_epoch)
    ) {
    ...
    if (all(
      ...
      ) && ((old_current_justified_checkpoint.epoch + 1uL) == current_epoch)
    ) {
      state.finalized_checkpoint = old_current_justified_checkpoint
    }
  }
  ...
  fun get_eligible_validator_indices(state: BeaconState): Sequence<ValidatorIndex> {
    var previous_epoch = get_previous_epoch(state)
    return enumerate(state.validators).filter { (_, v) ->
      is_active_validator(
        v,
        previous_epoch
      ) || v.slashed && ((previous_epoch + 1uL) < v.withdrawable_epoch)
    }.map { (index, _) -> ValidatorIndex(index) }.toMutableList()
  }

  /*
    Helper with shared logic for use by tech.pegasys.teku.phase1.onotole.pylib.get source, target, and head deltas functions
    */
  fun get_attestation_component_deltas(
    state: BeaconState,
    attestations: Sequence<PendingAttestation>
  ): Pair<Sequence<Gwei>, Sequence<Gwei>> {
  ...
  fun get_source_deltas(state: BeaconState): Pair<Sequence<Gwei>, Sequence<Gwei>> {
    var matching_source_attestations =
  ...
  fun get_head_deltas(state: BeaconState): Pair<Sequence<Gwei>, Sequence<Gwei>> {
    var matching_head_attestations =
  ...
  fun get_inclusion_delay_deltas(state: BeaconState): Pair<Sequence<Gwei>, Sequence<Gwei>> {
    ...
    for (index in get_unslashed_attesting_indices(state, matching_source_attestations)) {
      var attestation = min(matching_source_attestations.filter { a ->
        (index in get_attesting_indices(
          state,
          a.data,
          a.aggregation_bits
        ))
      }.map { a -> a }.toMutableList(), key = { a -> a.inclusion_delay })
  ...
  fun get_attestation_deltas(state: BeaconState): Pair<Sequence<Gwei>, Sequence<Gwei>> {
    ...
    var rewards =
      range(len(state.validators)).map { i -> (((source_rewards[i] + target_rewards[i]) + head_rewards[i]) + inclusion_delay_rewards[i]) }
        .toMutableList()
    var penalties =
      range(len(state.validators)).map { i -> (((source_penalties[i] + target_penalties[i]) + head_penalties[i]) + inactivity_penalties[i]) }
        .toMutableList()
    return Pair(rewards, penalties)
  }
  ...
  fun process_slashings(state: BeaconState): Unit {
    ...
    for ((index, validator) in enumerate(state.validators)) {
      if (validator.slashed && ((epoch + (EPOCHS_PER_SLASHINGS_VECTOR / 2uL)) == validator.withdrawable_epoch)) {
        var increment = EFFECTIVE_BALANCE_INCREMENT
        var penalty_numerator = ((validator.effective_balance / increment) * min(
          (sum(state.slashings) * 3uL),
          total_balance
        ))
  ...
  fun process_final_updates(state: BeaconState): Unit {
    ...
    if (((next_epoch % EPOCHS_PER_ETH1_VOTING_PERIOD) == 0uL)) {
      state.eth1_data_votes = SSZ.SSZList(
        Eth1Data::class,
    ...
    if (((next_epoch % (SLOTS_PER_HISTORICAL_ROOT / SLOTS_PER_EPOCH)) == 0uL)) {
      var historical_batch = DATA.HistoricalBatch(
    ...
    state.current_epoch_attestations = SSZ.SSZList(
  ...
  fun process_block_header(state: BeaconState, block: BeaconBlock): Unit {
    ...
    state.latest_block_header = DATA.BeaconBlockHeader(
      slot = block.slot,
  ...
  fun process_proposer_slashing(state: BeaconState, proposer_slashing: ProposerSlashing): Unit {
    ...
    for (signed_header in listOf(
      proposer_slashing.signed_header_1,
      proposer_slashing.signed_header_2
    )) {
      var domain =
        get_domain(state, DOMAIN_BEACON_PROPOSER, compute_epoch_at_slot(signed_header.message.slot))
  ...
  fun process_attestation(state: BeaconState, attestation: Attestation): Unit {
    ...
    var pending_attestation = DATA.PendingAttestation(
  ...
  fun process_deposit(state: BeaconState, deposit: Deposit): Unit {
    assert(
      is_valid_merkle_branch(
        ...
        depth = (DEPOSIT_CONTRACT_TREE_DEPTH + 1uL),
    ...
    if ((pubkey !in validator_pubkeys)) {
      var deposit_message = DATA.DepositMessage(
  ...
  fun get_forkchoice_store(anchor_state: BeaconState): Store {
    ...
    var justified_checkpoint = DATA.Checkpoint(epoch = anchor_epoch, root = anchor_root)
    var finalized_checkpoint = DATA.Checkpoint(epoch = anchor_epoch, root = anchor_root)
    return DATA.Store(
  ...
  fun get_latest_attesting_balance(store: Store, root: Root): Gwei {
    ...
    return Gwei(sum(active_indices.filter { i ->
      (i in store.latest_messages) && (get_ancestor(
        store,
  ...
  fun filter_block_tree(
    ...
    var children =
      store.blocks.keys().filter { root -> (store.blocks[root]!!.parent_root == block_root) }
        .map { root -> root }.toMutableList()
    if (any(children)) {
      var filter_block_tree_result =
        children.map { child -> filter_block_tree(store, child, blocks) }.toMutableList()
    ...
    var correct_justified =
      (store.justified_checkpoint.epoch == GENESIS_EPOCH) || (head_state.current_justified_checkpoint == store.justified_checkpoint)
    var correct_finalized =
      (store.finalized_checkpoint.epoch == GENESIS_EPOCH) || (head_state.finalized_checkpoint == store.finalized_checkpoint)
  ...
  fun get_head(store: Store): Root {
    ...
    while (true) {
      var children = blocks.keys()
        .filter { root -> (blocks[root]!!.parent_root == head) && (blocks[root]!!.slot > justified_slot) }
        .map { root -> root }.toMutableList()
      if ((len(children) == 0uL)) {
        return head
      }
      head =
        max(children, key = { root -> Tuple2(get_latest_attesting_balance(store, root), root) })
    }
  }
  ...
  /*
    ...
    */
  fun should_update_justified_checkpoint(
    store: Store,
  ...
  fun validate_on_attestation(store: Store, attestation: Attestation): Unit {
    ...
    var previous_epoch =
      if ((current_epoch > GENESIS_EPOCH)) (current_epoch - 1uL) else GENESIS_EPOCH
    ...
    for (i in attesting_indices) {
      if ((i !in store.latest_messages) || (target.epoch > store.latest_messages[i]!!.epoch)) {
        store.latest_messages[i] =
          DATA.LatestMessage(epoch = target.epoch, root = beacon_block_root)
      }
    }
  }
  ...
  fun on_tick(store: Store, time: uint64): Unit {
    ...
    if (!((current_slot > previous_slot) && (compute_slots_since_epoch_start(current_slot) == pyint(
        0uL
      )))
    ) {
      return
  ...
  fun on_block(store: Store, signed_block: SignedBeaconBlock): Unit {
    ...
    store.blocks[hash_tree_root(block)] = DATA.BeaconBlockHeader(
      slot = block.slot,
      proposer_index = block.proposer_index,
      parent_root = block.parent_root,
    ...
    if ((state.finalized_checkpoint.epoch > store.finalized_checkpoint.epoch)) {
      ...
      if ((state.current_justified_checkpoint.epoch > store.justified_checkpoint.epoch) || (get_ancestor(
          store,
          store.justified_checkpoint.root,
          finalized_slot
        ) != store.finalized_checkpoint.root)
  ...
  fun get_custody_atoms(bytez: pybytes): Sequence<pybytes> {
    val bytez_ = bytez + Bytes.fromHexString(
      "0x" + List(
        (BYTES_PER_CUSTODY_ATOM - (bytez.size().toUInt() % BYTES_PER_CUSTODY_ATOM)).toInt()
      ) { "00" })
    return range(0uL, bytez_.size().toULong(), BYTES_PER_CUSTODY_ATOM).map { i ->
      bytez_.slice(
        i,
        (i + BYTES_PER_CUSTODY_ATOM)
      )
    }.toMutableList()
  }
  ...
  fun compute_custody_bit(key: BLSSignature, data: pybytes): boolean {
    ...
    var a = sum(e1.map({ (i, atom) ->
      s[(i.toULong() % 2uL)].pow(i.toULong()) * from_bytes(
        atom,
        "little"
      )
    })) + s[(n % 2uL)].pow(n)
  ...
  fun get_randao_epoch_for_custody_period(period: uint64, validator_index: ValidatorIndex): Epoch {
    var next_period_start =
      (((period + 1uL) * EPOCHS_PER_CUSTODY_PERIOD) - (validator_index % EPOCHS_PER_CUSTODY_PERIOD))
  ...
  fun process_custody_key_reveal(state: BeaconState, reveal: CustodyKeyReveal): Unit {
    ...
    var epoch_to_sign = get_randao_epoch_for_custody_period(
      revealer.next_custody_secret_to_reveal,
      reveal.revealer_index
    )
    var custody_reveal_period =
      get_custody_period_for_validator(reveal.revealer_index, get_current_epoch(state))
    assert(
      (revealer.next_custody_secret_to_reveal < custody_reveal_period.value.toLong().toULong())
    )
  ...
  /*
    ...
    Note that this function mutates ``state``.
    */
  fun process_early_derived_secret_reveal(
    state: BeaconState,
    reveal: EarlyDerivedSecretReveal
  ): Unit {
    ...
    var signing_roots = mutableListOf(hash_tree_root(reveal.epoch), reveal.mask).map { root ->
    ...
    if ((reveal.epoch >= (get_current_epoch(state) + CUSTODY_PERIOD_TO_RANDAO_PADDING))) {
      ...
      var max_proposer_slot_reward =
        (((get_base_reward(state, reveal.revealed_index) * SLOTS_PER_EPOCH) / len(
          get_active_validator_indices(state, get_current_epoch(state))
        )) / PROPOSER_REWARD_QUOTIENT)
      var penalty = Gwei(
        ((max_proposer_slot_reward * EARLY_DERIVED_SECRET_REVEAL_SLOT_REWARD_MULTIPLE) * (len(state.exposed_derived_secrets[derived_secret_location]) + 1uL))
      )
      var proposer_index = get_beacon_proposer_index(state)
  ...
  fun process_custody_slashing(
    state: BeaconState,
    signed_custody_slashing: SignedCustodySlashing
  ): Unit {
    ...
    var epoch_to_sign = get_randao_epoch_for_custody_period(
      ...
      ).value.toLong().toULong(), custody_slashing.malefactor_index
    )
    ...
    var computed_custody_bit = compute_custody_bit(
      custody_slashing.malefactor_secret,
      Bytes.wrap(custody_slashing.data.toByteArray())
    )
    if ((claimed_custody_bit != computed_custody_bit)) {
      ...
      var whistleblower_reward =
        Gwei(((malefactor.effective_balance / WHISTLEBLOWER_REWARD_QUOTIENT) / others_count))
  ...
  fun process_custody_final_updates(state: BeaconState): Unit {
    state.exposed_derived_secrets[(get_current_epoch(state) % EARLY_DERIVED_SECRET_PENALTY_MAX_FUTURE_EPOCHS)] =
      DATA.ExposedValidatorIndices(max_size = MAX_EARLY_DERIVED_SECRET_REVEALS * SLOTS_PER_EPOCH)
  }
  ...
  /*
    ...
    */
  fun pack_compact_validator(
    index: ValidatorIndex,
    slashed: pybool,
    balance_in_increments: uint64
  ): uint64 {
  ...
  fun unpack_compact_validator(compact_validator: uint64): Triple<ValidatorIndex, pybool, uint64> {
    return Triple(
      ValidatorIndex((compact_validator shr 16uL)),
      pybool(((compact_validator shr 15uL) % 2uL)),
      (compact_validator and ((2uL.pow(15uL)) - 1uL))
    )
  }
  ...
  /*
    ...
    */
  fun committee_to_compact_committee(
    state: BeaconState,
    committee: Sequence<ValidatorIndex>
  ): CompactCommittee {
    var validators = committee.map { i -> state.validators[i] }.toMutableList()
    var compact_validators = zip(committee, validators).map { (i, v) ->
      pack_compact_validator(
        i,
        v.slashed,
        (v.effective_balance / EFFECTIVE_BALANCE_INCREMENT)
      )
    }.toMutableList()
    var pubkeys = validators.map { v -> v.pubkey }.toMutableList()
    return DATA.CompactCommittee(
      pubkeys = SSZ.SSZList(
        BLSPubkey::class,
        MAX_VALIDATORS_PER_COMMITTEE,
        pubkeys
      ),
      compact_validators = SSZ.SSZList(
        uint64::class,
        MAX_VALIDATORS_PER_COMMITTEE,
        compact_validators
      )
  ...
  fun compute_shard_from_committee_index(
    state: BeaconState,
    index: CommitteeIndex,
    slot: Slot
  ): Shard {
  ...
  fun compute_offset_slots(start_slot: Slot, end_slot: Slot): Sequence<Slot> {
    return SHARD_BLOCK_OFFSETS.filter { x -> ((start_slot + x) < end_slot) }
      .map { x -> Slot((start_slot + x)) }.toMutableList()
  }

  fun compute_updated_gasprice(prev_gasprice: Gwei, length: uint8): Gwei {
    if ((length > TARGET_SHARD_BLOCK_SIZE)) {
      var delta =
        (((prev_gasprice * (length - TARGET_SHARD_BLOCK_SIZE)) / TARGET_SHARD_BLOCK_SIZE) / GASPRICE_ADJUSTMENT_COEFFICIENT)
      return min((prev_gasprice + delta), MAX_GASPRICE)
    } else {
      var delta =
        (((prev_gasprice * (TARGET_SHARD_BLOCK_SIZE - length)) / TARGET_SHARD_BLOCK_SIZE) / GASPRICE_ADJUSTMENT_COEFFICIENT)
  ...
  fun get_online_validator_indices(state: BeaconState): Set<ValidatorIndex> {
    ...
    return set(active_validators.filter { i -> (state.online_countdown[i] != 0u.toUByte()) }
      .map { i -> i }.toMutableList())
  }

  fun get_shard_committee(
    beacon_state: BeaconState,
    epoch: Epoch,
    shard: Shard
  ): Sequence<ValidatorIndex> {
  ...
  fun get_light_client_committee(
    beacon_state: BeaconState,
    epoch: Epoch
  ): Sequence<ValidatorIndex> {
    ...
    ).slice(0uL, TARGET_COMMITTEE_SIZE)
  }

  fun get_shard_proposer_index(
    beacon_state: BeaconState,
    slot: Slot,
    shard: Shard
  ): ValidatorIndex {
    ...
    var r = bytes_to_int(
      get_seed(
        beacon_state,
        get_current_epoch(beacon_state),
  ...
  fun validate_attestation(state: BeaconState, attestation: Attestation): Unit {
    ...
    if ((attestation.custody_bits_blocks != SSZ.SSZList(
        SSZBitList::class,
        MAX_VALIDATORS_PER_COMMITTEE
      ))
    ) {
      ...
      assert(
        (data.beacon_block_root == get_block_root_at_slot(
          state,
          compute_previous_slot(state.slot)
        ))
      )
    } else {
      assert(((data.slot + MIN_ATTESTATION_INCLUSION_DELAY) < state.slot))
  ...
  fun apply_shard_transition(state: BeaconState, shard: Shard, transition: ShardTransition): Unit {
    ...
    assert(
      (len(transition.shard_data_roots) == len(transition.shard_states)) && (len(transition.shard_states) == len(
        transition.shard_block_lengths
      )) && (len(transition.shard_block_lengths) == len(offset_slots))
    )
    ...
    for (i in range(len(offset_slots))) {
      ...
      assert(
        (shard_state.gasprice == compute_updated_gasprice(
          prev_gasprice,
          shard_block_length.toUByte()
        ))
      )
      ...
      if (!(is_empty_proposal)) {
        ...
        var header = DATA.ShardBlockHeader(
    ...
    var signing_roots = headers.map { header ->
      compute_signing_root(
        header,
        get_domain(state, DOMAIN_SHARD_PROPOSAL, compute_epoch_at_slot(header.slot))
      )
    }.toMutableList()
    assert(
      bls.AggregateVerify(
        zip(pubkeys, signing_roots),
  ...
  fun process_crosslink_for_shard(
    state: BeaconState,
    committee_index: CommitteeIndex,
    ...
    var shard_transition_roots =
      set(attestations.map { a -> a.data.shard_transition_root }.toMutableList())
    for (shard_transition_root in sorted(shard_transition_roots)) {
      var transition_attestations =
        attestations.filter { a -> (a.data.shard_transition_root == shard_transition_root) }
          .map { a -> a }.toMutableList()
      ...
      for (attestation in transition_attestations) {
        ...
        assert(
          (attestation.data.shard_head_root == shard_transition.shard_data_roots[(len(
            shard_transition.shard_data_roots
          ) - 1uL)])
        )
      }
      var enough_online_stake = ((get_total_balance(
        state,
        online_indices.intersection(transition_participants)
      ) * 3uL) >= (get_total_balance(state, online_indices.intersection(committee)) * 2uL))
      ...
      var estimated_attester_reward =
        sum(transition_participants.map { attester -> get_base_reward(state, attester) }
          .toMutableList())
      var proposer_reward = Gwei((estimated_attester_reward / PROPOSER_REWARD_QUOTIENT))
      increase_balance(state, beacon_proposer_index, proposer_reward)
      var states_slots_lengths = zip(
    ...
    assert((shard_transition == DATA.ShardTransition()))
    return Root()
  }

  fun process_crosslinks(
    state: BeaconState,
    ...
    for (committee_index in map(::CommitteeIndex, range(committee_count))) {
      ...
      var shard_attestations = attestations.filter { attestation ->
        is_shard_attestation(
          state,
          attestation,
          committee_index
        )
      }.map { attestation -> attestation }.toMutableList()
      var winning_root =
        process_crosslink_for_shard(state, committee_index, shard_transition, shard_attestations)
  ...
  fun get_indices_from_committee(
    committee: SSZList<ValidatorIndex>,
    bits: SSZBitList
  ): SSZMutableList<ValidatorIndex> {
    assert((len(bits) == len(committee)))
    return SSZ.SSZList(
      ValidatorIndex::class,
      MAX_VALIDATORS_PER_COMMITTEE,
      enumerate(committee).filter { (i, _) -> bits[i] }
        .map { (_, validator_index) -> validator_index }.toMutableList()
    )
  }

  fun verify_shard_transition_false_positives(
    state: BeaconState,
    block_body: BeaconBlockBody
  ): Unit {
    for (shard in range(get_active_shard_count(state))) {
      if ((state.shard_states[shard].slot != (state.slot - 1uL))) {
        assert((block_body.shard_transitions[shard] == DATA.ShardTransition()))
      }
    }
  }
  ...
  fun process_light_client_signatures(state: BeaconState, block_body: BeaconBlockBody): Unit {
    ...
    increase_balance(
      ...
      Gwei((total_reward / PROPOSER_REWARD_QUOTIENT))
    )
    var slot = compute_previous_slot(state.slot)
    var signing_root = compute_signing_root(
  ...
  fun process_light_client_committee_updates(state: BeaconState): Unit {
    if (((get_current_epoch(state) % LIGHT_CLIENT_COMMITTEE_PERIOD) == 0uL)) {
      ...
      var new_committee = get_light_client_committee(
        state,
  ...
  fun compute_shard_transition_digest(
    beacon_state: BeaconState,
  ...
  fun verify_shard_block_message(
    beacon_state: BeaconState,
    shard_state: ShardState,
    block: ShardBlock,
    slot: Slot,
  ...
  fun verify_shard_block_signature(
    beacon_state: BeaconState,
  ...
  fun is_valid_fraud_proof(
    ...
    block: ShardBlock,
    subkey: BLSPubkey,
    beacon_parent_block: BeaconBlock
  ): pybool {
  ...
  fun get_winning_proposal(
    beacon_state: BeaconState,
    proposals: Sequence<SignedShardBlock>
  ): SignedShardBlock {
  ...
  /*
    ...
    */
  fun get_proposal_choices_at_slot(
    beacon_state: BeaconState,
    shard_state: ShardState,
    slot: Slot,
    shard: Shard,
    shard_blocks: Sequence<SignedShardBlock>,
    validate_signature: pybool = true
  ): Sequence<SignedShardBlock> {
    var choices = mutableListOf<SignedShardBlock>()
    var shard_blocks_at_slot =
      shard_blocks.filter { block -> (block.message.slot == slot) }.map { block -> block }
        .toMutableList()
  ...
  fun get_proposal_at_slot(
    ...
    shard_blocks: Sequence<SignedShardBlock>,
    validate_signature: pybool = true
  ): Pair<SignedShardBlock, ShardState> {
    var choices = get_proposal_choices_at_slot(
    ...
    if ((len(choices) == 0uL)) {
      var block = DATA.ShardBlock(slot = slot)
      proposal = DATA.SignedShardBlock(message = block)
    } else {
      if ((len(choices) == 1uL)) {
  ...
  fun get_shard_state_transition_result(
    beacon_state: BeaconState,
    shard: Shard,
    shard_blocks: Sequence<SignedShardBlock>,
    validate_signature: pybool = true
  ): Triple<Sequence<SignedShardBlock>, Sequence<ShardState>, Sequence<Root>> {
    ...
    for (slot in get_offset_slots(beacon_state, shard)) {
      val (proposal, shard_state_) = get_proposal_at_slot(
        beacon_state = beacon_state,
        shard_state = shard_state,
        slot = slot,
        shard = shard,
  ...
  fun get_shard_transition(
    ...
    val (proposals, shard_states, shard_data_roots) = get_shard_state_transition_result(
      beacon_state,
      shard,
      shard_blocks
    )
    ...
    return DATA.ShardTransition(
      start_slot = start_slot,
      shard_block_lengths = SSZ.SSZList(
        uint64::class,
        MAX_SHARD_BLOCKS_PER_ATTESTATION,
        shard_block_lengths
      ),
      shard_data_roots = SSZ.SSZList(
        Bytes32::class,
        MAX_SHARD_BLOCKS_PER_ATTESTATION,
        shard_data_roots.toMutableList()
      ),
      shard_states = SSZ.SSZList(
        ShardState::class,
        MAX_SHARD_BLOCKS_PER_ATTESTATION,
        shard_states.toMutableList()
      ),
      proposer_signature_aggregate = proposer_signature_aggregate
    )
  }

  fun tech.pegasys.teku.phase1.onotole.phase0.BeaconBlockHeader.toPhase1() = DATA.BeaconBlockHeader(
    slot = this.slot,
    proposer_index = this.proposer_index,
    parent_root = this.parent_root,
    state_root = this.state_root,
    body_root = this.body_root
  )

  fun tech.pegasys.teku.phase1.onotole.phase0.Eth1Data.toPhase1() = DATA.Eth1Data(
    deposit_root = this.deposit_root,
  ...
  fun upgrade_to_phase1(pre: tech.pegasys.teku.phase1.onotole.phase0.BeaconState): BeaconState {
    ...
    var post = DATA.BeaconState(
      ...
      eth1_data_votes = SSZ.SSZList(
        Eth1Data::class,
        EPOCHS_PER_ETH1_VOTING_PERIOD * SLOTS_PER_EPOCH,
        pre.eth1_data_votes.map { it.toPhase1() }.toMutableList()
      ),
      eth1_deposit_index = pre.eth1_deposit_index,
      validators = SSZ.SSZList(
        Validator::class,
        VALIDATOR_REGISTRY_LIMIT,
        enumerate(pre.validators).map { (i, phase0_validator) ->
          DATA.Validator(
      ...
      shard_states = SSZ.SSZList(
        ShardState::class,
        MAX_SHARDS,
        range(INITIAL_ACTIVE_SHARDS).map {
          DATA.ShardState(
            slot = pre.slot,
      ...
      online_countdown = SSZ.SSZList(
        OnlineEpochs::class,
        VALIDATOR_REGISTRY_LIMIT,
        List(len(pre.validators).toInt()) { mutableListOf(ONLINE_PERIOD) }.flatten().toMutableList()
      ),
      current_light_committee = DATA.CompactCommittee(),
      next_light_committee = DATA.CompactCommittee(),
      exposed_derived_secrets = SSZ.SSZVector(
        ExposedValidatorIndices::class,
        range(EARLY_DERIVED_SECRET_PENALTY_MAX_FUTURE_EPOCHS).map {
          DATA.ExposedValidatorIndices(max_size = MAX_EARLY_DERIVED_SECRET_REVEALS * SLOTS_PER_EPOCH)
        }.toMutableList()
      )
    )
    var next_epoch = Epoch((epoch + 1uL))
    post.current_light_committee =
      committee_to_compact_committee(post, get_light_client_committee(post, epoch))
    post.next_light_committee =
      committee_to_compact_committee(post, get_light_client_committee(post, next_epoch))
    return post
  }
