...
public final class BlockProcessorUtil {
  ...
  public void processOperationsNoValidation(MutableBeaconState state, BeaconBlockBody body)
    ...
    try {
      ...
      processAttestationsNoValidation(
          state,
          SSZList.createMutable(
              body.getAttestations().stream(), Constants.MAX_ATTESTATIONS, Attestation.class));
      processDeposits(
          state,
          SSZList.createMutable(
              body.getDeposits().stream(), Constants.MAX_DEPOSITS, Deposit.class));
